...
features1 <- gaus_basis(mean, sd)
df1 <- cbind(my_values, features1)
df <- cbind(rep(1, 100), unname(as.matrix(df1[, - c(1:2)])))

features1_test <- gaus_basis(mean, sd)
df1_test <- cbind(my_values_test, features1_test)
df_test <- cbind(rep(1, 100), unname(as.matrix(df1_test[, - c(1:2)])))
...
library(glmnet) # we use this library to determine the optimal lambda

lambdas <- 10^seq(2, -2, by = -0.1)
ridge1 <- glmnet(df[1:20, ], as.numeric(df1[1:20, 2]), 
                   alpha = 0, lambda = lambdas)
opt_lambda1 <- ridge1[[5]][length(ridge1[[5]])] # cross-validation with 20 observations does not produce nice results, 
                                    # thus the minimizing value for explained deviance was taken
...
# now we calculate our own ridge regression with the lambda already predetermined
...
# result overview and some graphs
print(cbind(rls1, rls2, rls3, rls4, rls5))
...
# we chose as prior alpha = 0 and beta equal 20, we can see the results are quite good
...
# our results plus some graphs
print(cbind(bayes1, bayes2, bayes3, bayes4, bayes5))
...
ggplot(data = df1) +
  ...
  geom_line(aes(x = x, 
                y = fitted_bayes5), color = "gray") +
  theme_minimal()

##### Comparison of Ridge and Bayesian regression #####
ggplot(data = df1) +
  geom_jitter(aes(x = x, y = target)) +
  geom_line(aes(x = x, 
                y = fitted_rls1), color = "red") +
...
##### Bayesian regression, estimating alpha, beta #####
...
fitted_BLR1 <- df %*% as.matrix(BLR1)
BLR2 <- BLR_optim(df[1:40, ], df1$target[1:40])
fitted_BLR2 <- df %*% as.matrix(BLR2)
BLR3 <- BLR_optim(df[1:60, ], df1$target[1:60])
fitted_BLR3 <- df %*% as.matrix(BLR3)
BLR4 <- BLR_optim(df[1:80, ], df1$target[1:80])
fitted_BLR4 <- df %*% as.matrix(BLR4)
BLR5 <- BLR_optim(df, df1$target)
fitted_BLR5 <- df %*% as.matrix(BLR5)

print(cbind(BLR1, BLR2, BLR3, BLR4, BLR5))
